...
def completed_epoch_times(history: str) -> dict[int, float]:
    ...
    for block in history.split("╭"):
        ...
        if epoch and uptime:
            result[int(epoch[1])] = (
                int(uptime[1]) * 60 + int(uptime[2]) + int(uptime[3]) / 1000
            )
    return result


def interval_sps(times: dict[int, float], span: int) -> float | None:
    if not times:
        return None
    last = max(times)
    first = last - span
...
def main() -> None:
    ...
    sps_16 = interval_sps(times, 16)
    sps_20 = interval_sps(times, 20)
    print(
    ...
    if not completed and startup_seconds >= 600 and epoch == 0:
        raise SystemExit("No completed training epoch after 600 seconds")
    if (
        not completed and epoch >= 30
        and sps_16 is not None and sps_20 is not None
        and sps_16 < MIN_SPS and sps_20 < MIN_SPS
    ):
        raise SystemExit("Sustained end-to-end training SPS below 25,000")
