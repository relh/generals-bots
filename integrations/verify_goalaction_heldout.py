"""Audit the final goal-feature Fabric policy and independent classic-map evaluation."""
...
def audit_evaluation(path: Path, record: dict, digest: str, seeds: list[int]) -> dict:
    ...
    require(build["fabric"]["factory"] == "integrations.generals_fabric:tied_local_action_policy", f"{path}: wrong policy")
    ...
    require(
        ...
        and options["goal_features"] is True
        and options["parallel_games"] == spec["agents"] == 1024
        and spec["observation_size"] == 2100,
...
def audit(workspace: Path, training_job: int, validation_job: int, heldout_job: int) -> dict:
    ...
    for index in range(2):
        run = workspace / f"goalaction-two-30m-{training_job}-{index}"
        ...
        validation = audit_evaluation(
            workspace / f"goalaction-validation-{validation_job}{suffix}",
    ...
    selection = read(workspace / f"goalaction-heldout-selection-{heldout_job}.json")
    ...
    heldout = audit_evaluation(
        workspace / f"goalaction-heldout-{heldout_job}",
