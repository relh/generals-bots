...
def test_representative_selection_cycles_opponent_groups():
    rows = [dict(suite='open4', opponent=opponent, board_id=str(i), result='loss')
            for opponent in ('hunter', 'random') for i in range(3)]
...
def test_snapshot_provenance_exempts_current_candidate_but_not_core_changes(tmp_path, monkeypatch):
    ...
    metadata = dict(source_hashes={'generals/agents/sentinel_agent.py': 'old', 'generals/core/game.py': 'old'},
