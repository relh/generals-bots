...
def make_suite(name, seed, count):
    ...
    if competition:
        ...
        dims = [(int(jax.random.randint(jax.random.PRNGKey(seed + i), (), 18, 22)),
                 int(jax.random.randint(jax.random.fold_in(jax.random.PRNGKey(seed + i), 1), (), 18, 22)))
                for i in range(count)]
    else:
        size = {'classic8': 8, 'classic12': 12}[name]
        env = GeneralsEnv(grid_dims=(size, size), min_generals_distance=size - 2,
                          num_castles_range=(2, 5), castle_val_range=(20, 41), truncation=800)
    ...
    def board(key, h, w):
        grid = generate_grid(key, grid_dims=(h, w), pad_to=max(h, w),
                             ...
                             castle_val_range=env.castle_val_range)[:h, :w]
    ...
    for board_id, grid in enumerate(boards):
        for repeat in range(repeats):
            ...
            for swapped in (False, True):
                ...
                for seat in (0, 1):
                    cases.append(dict(board_id=board_id, repeat=repeat, swapped=int(swapped),
