...
from .cli import V3_OPTIONS, V4_OPTIONS, V5_OPTIONS, V6_OPTIONS, V7_OPTIONS, V8_OPTIONS, agent
...
def make_policy(name, rules, checkpoint=None, source=None, *, options=None):
    if source:
        if (
            name != "sentinel"
            and name not in V3_OPTIONS | V4_OPTIONS | V5_OPTIONS | V6_OPTIONS | V7_OPTIONS | V8_OPTIONS
        ):
        ...
        player = player_class(
            ...
            **(
                (V3_OPTIONS | V4_OPTIONS | V5_OPTIONS | V6_OPTIONS | V7_OPTIONS | V8_OPTIONS).get(name, {})
                | (options or {})
            ),
        )
        if name in V3_OPTIONS | V6_OPTIONS | V7_OPTIONS | V8_OPTIONS:
            return player, None
...
def source_provenance(metadata, row, candidate_source=None):
    ...
    critical = [
        ...
        or (
            any(
                policy in V3_OPTIONS | V4_OPTIONS | V5_OPTIONS | V6_OPTIONS | V7_OPTIONS | V8_OPTIONS
        ...
        or (
            any(
                policy in V4_OPTIONS | V5_OPTIONS | V6_OPTIONS | V7_OPTIONS | V8_OPTIONS
                for policy in (row["candidate"], row["opponent"])
            )
            and name == "generals/agents/sentinel_v3_agent.py"
        )
        or (
            any(policy in V6_OPTIONS | V7_OPTIONS | V8_OPTIONS for policy in (row["candidate"], row["opponent"]))
            and name == "generals/agents/sentinel_v5_agent.py"
        )
        or (
            any(policy in V7_OPTIONS | V8_OPTIONS for policy in (row["candidate"], row["opponent"]))
            and name == "generals/agents/sentinel_v6_agent.py"
        )
        or (
            any(policy in V8_OPTIONS for policy in (row["candidate"], row["opponent"]))
