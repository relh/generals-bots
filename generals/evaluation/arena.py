...
def valid_commands(actions, shape, rules):
    ...
    move_ok = ((kind == 0) & source_ok & (direction >= 0) & (direction < 4)
               & ((split == 0) | (split == 1)))
...
def action_counters(before, after, actions, rules=Rules()):
    ...
    valid = ((r >= 0) & (r < h) & (c >= 0) & (c < w)
             ...
             & before.ownership[players, rs, cs] & (before.armies[rs, cs] > 1)
    ...
    return jnp.stack([kind == 1, (kind == 0) & (split == 1), kind == 2,
                      built, (kind == 0) & ~(valid & command_ok),
                      ~command_ok], axis=-1).astype(jnp.int32)
...
def make_runner(candidate, opponent, rules=Rules(), *, from_states=False):
    ...
    def run(grids, keys, seats):
        ...
        def body(carry):
            ...
            actions = jnp.stack([jnp.where(seats[:, None] == 0, ours, theirs),
                                 jnp.where(seats[:, None] == 0, theirs, ours)], axis=1)
