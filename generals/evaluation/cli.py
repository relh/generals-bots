...
def agent(name, rules, checkpoint=None):
    if name == 'sentinel':
        ...
        return SentinelAgent(build_castles=rules.build_castles,
                             deathtouch_turn=rules.deathtouch_turn, max_turns=rules.max_turns).act
    if name == 'learned':
    ...
    return {'random': RandomAgent, 'expander': ExpanderAgent,
            'hunter': HunterAgent, 'harvester': HarvesterAgent}[name]().act
...
def summary(rows):
    ...
    return dict(games=len(rows), wins=sum(r['result'] == 'win' for r in rows),
                losses=sum(r['result'] == 'loss' for r in rows), draws=sum(r['result'] == 'draw' for r in rows),
                score=float(scores.mean()), score_ci95=np.quantile(bootstrap, [.025, .975]).tolist(),
...
def main():
    ...
    parser.add_argument('--candidate', default='sentinel', choices=['sentinel', 'learned', 'old-ppo', 'random', 'expander', 'hunter', 'harvester'])
    ...
    paths = [p for area in ('agents', 'core', 'modifiers', 'evaluation', 'training')
    ...
    summaries = {}
    fields = ['suite', 'opponent', 'candidate', 'board_id', 'repeat', 'swapped', 'seat', 'action_seed',
              'height', 'width', 'result', 'score', 'winner', 'turns', 'terminal',
              'own_passes', 'own_splits', 'own_build_attempts', 'own_builds', 'own_invalid_moves',
              'enemy_passes', 'enemy_splits', 'enemy_build_attempts', 'enemy_builds', 'enemy_invalid_moves',
              'own_malformed_commands', 'enemy_malformed_commands']
                ...
                for shape_cases in by_shape.values():
                    for offset in range(0, len(shape_cases), args.batch_size):
                        ...
                        for i, case in enumerate(batch):
                            ...
                            row.update(suite=suite, opponent=opponent, candidate=args.candidate,
                                       height=case['grid'].shape[0], width=case['grid'].shape[1],
                                       result=outcome, score={'win': 1., 'draw': .5, 'loss': 0.}[outcome],
                                       winner=winner, turns=int(result.state.time[i]), terminal=bool(result.finished[i]))
                            for prefix, player in [('own', seat), ('enemy', 1-seat)]:
                                for metric, value in zip(['passes', 'splits', 'build_attempts', 'builds', 'invalid_moves', 'malformed_commands'], result.counters[i, player]):
