...
def matchup_report(rows):
    ...
    return dict(
        ...
        gate=bool(len(means) >= 64 and rate >= .85 and low > .70),
    )
...
def main():
    ...
    expected_cases = {(str(board), str(repeat), str(swapped), str(seat))
                      for board in range(metadata['boards']) for repeat in range(metadata['repeats'])
                      for swapped in (0, 1) for seat in (0, 1)}
    complete = set(groups) == expected and all(
        len(group) == len(expected_cases) and
        {(row['board_id'], row['repeat'], row['swapped'], row['seat']) for row in group} == expected_cases
