...
def compute_gae(rewards, values, next_values, terminated, truncated, gamma=0.99, lam=0.95):
    ...
    return jax.lax.scan(step, jnp.zeros_like(values[0]), (deltas, terminated | truncated), reverse=True)[1]
...
def loss(network, batch, clip=0.2, entropy_weight=0.01, value_weight=0.5):
    ...
    policy = -jnp.minimum(ratio * batch.advantages, jnp.clip(ratio, 1 - clip, 1 + clip) * batch.advantages).mean()
