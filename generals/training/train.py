...
def make_env(config, stage):
    ...
    return GeneralsEnv(grid_dims=(size, size), mode=config.mode, pool_size=config.pool_size,
                       truncation=config.truncation, build_castles=config.build_castles,
                       mountain_density_range=(0.0, config.terrain_density), num_castles_range=(0, 3),
                       min_generals_distance=min(3, size - 1), castle_val_range=(10, 25))
...
def outcome_counts(terminated, truncated, outcomes):
    ...
    return dict(completed_episodes=int(done.sum()), wins=int((done & (outcomes > 0)).sum()),
                losses=int((done & (outcomes < 0)).sum()), draws=int((done & (outcomes == 0)).sum()),
                timeouts=int(truncated.sum()), terminal_draws=int((terminated & (outcomes == 0)).sum()))
...
def opponents_for(config, env):
    ...
    if "sentinel" in names:
        ...
        available["sentinel"] = SentinelAgent(build_castles=env.build_castles,
...
def make_rollout(env, opponents, config):
    ...
    def rollout(network, pool, states, sides, ids, episode_returns, key):
        def step(carry, unused):
            ...
            actions = jnp.where(sides[:, None, None] == 0,
                                jnp.stack([ours, theirs], 1), jnp.stack([theirs, ours], 1))
            ...
            rewards, components = jax.vmap(lambda old, new, winner, team, terminal:
                shaped_reward(old, new, winner, team, terminal, config.gamma, config.shaping_scale))(
            ...
            data = dict(observations=inputs, masks=masks, actions=indices, logprobs=logprobs,
                        values=values, next_values=next_values, rewards=rewards,
                        terminated=timesteps.terminated, truncated=timesteps.truncated,
                        bootstrap_terminal=bootstrap_terminal,
                        outcome=components[0], shaping=components[1], potential=components[2],
                        episode_return=total_returns, episode_length=timesteps.last_state.time,
                        opponent=ids, side=sides)
            return (next_states, jnp.where(done, new_sides, sides), jnp.where(done, new_ids, ids),
                    jnp.where(done, 0, total_returns), key), data
...
def initialize(config):
    ...
    return dict(config=asdict(config), network=network, optimizer=optimizer.init(eqx.filter(network, eqx.is_array)),
                rng=key, pool=pool, states=states, sides=jax.random.randint(side_key, (config.num_envs,), 0, 2),
                opponent_ids=jax.random.randint(opponent_key, (config.num_envs,), 0, len(config.opponents.split(","))),
                episode_returns=jnp.zeros(config.num_envs), iteration=0, stage=0, environment_steps=0,
                episodes=0, metadata=dict(jax_version=jax.__version__, device=str(jax.devices()[0]),
                                         observation="public fog-of-war", algorithm="spatial-ppo-v1",
    ...
    with (output / "metrics.jsonl").open("a") as metrics, (output / "episodes.jsonl").open("a") as episodes:
        for iteration in range(snapshot["iteration"], config.iterations):
            ...
            if stage != snapshot["stage"]:
                ...
                snapshot["states"] = jax.tree.map(lambda x: x[jnp.arange(config.num_envs) % env.pool_size], snapshot["pool"])
                ...
                metrics.write(json.dumps(dict(event="curriculum_reset", iteration=iteration,
                                              discarded_episodes=config.num_envs, stage=stage)) + "\n")
            elif iteration > 0 and config.pool_refresh and iteration % config.pool_refresh == 0:
                snapshot["rng"], pool_key = jax.random.split(snapshot["rng"])
                snapshot["pool"], _ = env.reset(pool_key)
            carry, data = rollout(snapshot["network"], snapshot["pool"], snapshot["states"], snapshot["sides"],
                                   snapshot["opponent_ids"], snapshot["episode_returns"], snapshot["rng"])
            (snapshot["states"], snapshot["sides"], snapshot["opponent_ids"],
             snapshot["episode_returns"], snapshot["rng"]) = carry
            jax.block_until_ready(data)
            rollout_seconds = time.perf_counter() - start
            advantages = compute_gae(data["rewards"], data["values"], data["next_values"],
                                      data["bootstrap_terminal"], data["truncated"], config.gamma, config.gae_lambda)
            returns = advantages + data["values"]
            advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-8)
            n = config.num_envs * config.steps
            batch = Batch(*(x.reshape(n, *x.shape[2:]) for x in
                            (data["observations"], data["masks"], data["actions"], data["logprobs"], advantages, returns)))
            diagnostics = []
            stop_early = False
            for epoch in range(config.epochs):
                ...
                for begin in range(0, n, config.minibatch_size):
                    ...
                    snapshot["network"], snapshot["optimizer"], diagnostic = update(
                        snapshot["network"], snapshot["optimizer"], minibatch, optimizer,
                        config.clip, config.entropy_weight)
                ...
                if stop_early:
                    break
            host = {k: np.asarray(v) for k, v in data.items() if k not in
                    ("observations", "masks", "actions", "logprobs", "values", "next_values")}
            done = host["terminated"] | host["truncated"]
            for step_index, env_index in zip(*np.nonzero(done)):
                ix = (step_index, env_index)
                episodes.write(json.dumps(dict(iteration=iteration + 1, environment=int(env_index),
                    opponent=config.opponents.split(",")[int(host["opponent"][ix])], side=int(host["side"][ix]),
                    outcome=float(host["outcome"][ix]), truncated=bool(host["truncated"][ix]),
                    length=int(host["episode_length"][ix]), shaped_return=float(host["episode_return"][ix]))) + "\n")
            ...
            explained_variance = (1.0 - float(jnp.var(returns - data["values"])) / return_variance
                                  if return_variance > 1e-12 else None)
            cells = data["observations"].shape[-2] * data["observations"].shape[-1]
            row = dict(iteration=iteration + 1, stage=stage, environment_steps=snapshot["environment_steps"],
                       episodes=snapshot["episodes"],
                       **outcome_counts(host["terminated"], host["truncated"], host["outcome"]),
                       reward_mean=float(host["rewards"].mean()),
                       outcome_mean=float(host["outcome"].mean()), shaping_mean=float(host["shaping"].mean()),
                       potential_mean=float(host["potential"].mean()), rollout_seconds=rollout_seconds,
                       pass_fraction=float((data["actions"] == 9 * cells).mean()),
                       build_fraction=float(((data["actions"] >= 8 * cells) & (data["actions"] < 9 * cells)).mean()),
                       explained_variance=explained_variance,
                       update_seconds=elapsed - rollout_seconds, steps_per_second=n / elapsed,
                       optimizer_steps=len(diagnostics), kl_early_stop=stop_early, **diagnostic)
        ...
        print(json.dumps(dict(event="complete", iteration=snapshot["iteration"],
                              checkpoint=str(output / "checkpoint.pkl"))), flush=True)
    return snapshot
...
def main():
    ...
    for name, value in asdict(defaults).items():
        if isinstance(value, bool):
            ...
            parser.add_argument("--" + name.replace("_", "-"), type=type(value) if value is not None else str, default=value)
    ...
    if resume:
        ...
        config = saved_config
    for name in ("num_envs", "steps", "iterations", "pool_size", "stage_iterations", "epochs", "minibatch_size", "save_every"):
