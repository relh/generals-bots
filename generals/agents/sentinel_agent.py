...
class SentinelAgent(Agent):
    ...
    def __init__(self, id="Sentinel", *, build_castles=False, deathtouch_turn=None,
                 max_turns=1200):
        super().__init__(id)
    ...
    def decision(self, obs: Observation, key):
        ...
        active_touch = (obs.timestep >= self.deathtouch_turn
        ...
        passable = ~(obs.mountains | obs.structures_in_fog |
                     (obs.castles & obs.neutral_cells & ~affordable))
        ...
        near_force = jnp.max(jnp.where(obs.opponent_cells & (home_distance <= 3),
                                     jnp.maximum(a - home_distance, 0), 0))
        ...
        reserve_force = jnp.max(jnp.where(
            obs.opponent_cells & (home_distance <= 3),
            jnp.maximum(a - home_distance, 0) / jnp.maximum(home_distance, 1), 0))
        ...
        city_value = jnp.where(affordable & (home_distance < 1e5),
                              50 - a * .4 - home_distance, -1e6)
        ...
        goal = jnp.where(jnp.any(egen), egen,
                        jnp.where(take_city, city_target,
                                  jnp.where(jnp.any(enemy), enemy, farthest)))
        costs = 1 + jnp.where(~friendly, a, 0) * .12
        ...
        valid = (compute_valid_move_mask_obs(obs)[..., None] &
                 ~_neighbors(obs.structures_in_fog, True)[..., None] & (moved > 0))
        ...
        scores = (advances[..., None] * (5 + moved * .65) +
                  captures * (3 + dest_enemy * 3 + dest_castle * 18) +
                  dest_fog * 1.5 + dest_mine * jnp.minimum(dest_a, moved) * .10 -
                  jnp.where(captures, dest_a * .12, 0) -
                  jnp.maximum(-safety, 0) * .8)
        ...
        removed_threat = (dest_enemy & captures &
                          (_neighbors(home_distance, 1e6)[..., None] == 1) &
                          ~gen[..., None, None])
        second_threat = jnp.sort(jnp.where(immediate, a - 1, 0).reshape(-1))[-2]
        after_threat = jnp.where(removed_threat & (dest_a - 1 >= imminent_army),
                                 second_threat, imminent_army)
        after_home = (gen_army - gen[..., None, None] * moved +
                      dest_gen * moved)
        ...
        if self.build_castles:
            ...
            safe_build = (mine & ~own_structures & (a >= price + 5 + local_threat) &
                          (home_distance >= 2) & (near_force < gen_army) &
                          (obs.timestep + 2 * price + 100 < self.max_turns))
            build_score = jnp.where(safe_build, 45 + (self.max_turns - obs.timestep) * .08 - price * .3, -1e9)
        ...
        telemetry = {"score": score, "general_army": gen_army,
                     "general_reserve": reserve, "adjacent_threat": imminent_army,
                     "goal_visible_general": jnp.any(egen), "goal_city": take_city,
                     "building": building, "deathtouch_active": active_touch,
